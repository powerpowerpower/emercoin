...
Source:		https://github.com/%{name}/%{name}/releases/download/v%{version}emc/%{name}-%{version}-i686-pc-linux-gnu.tar.gz
BuildArch:	i686 
Requires:	openssl
Requires:	miniupnpc
Requires:	qt5-qtbase
...
cd %{_builddir}
rm -rf %{name}-%{version}
tar -xvvf %{_sourcedir}/%{name}-%{version}-i686-pc-linux-gnu.tar.gz
cd %{name}-%{version}
